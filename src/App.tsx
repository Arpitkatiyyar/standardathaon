...
function HomePage({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'start' })
      setTimeout(() => window.scrollBy(0, -96), 300)
    }
  }, [location.pathname])

  return (
    <>
      <div ref={homeRef} className="min-h-screen pt-20"><HeroSection onNavigate={onNavigate} /></div>
      <div ref={aboutRef} className="min-h-screen pt-20"><AboutSection /></div>
      <div ref={timelineRef} className="min-h-screen pt-20"><TimelineSection /></div>
      <div ref={eligibilityRef} className="min-h-screen pt-20"><Eligibility /></div>
      <div ref={membersRef} className="min-h-screen pt-20"><Members /></div>
      <div ref={problemsRef} className="min-h-screen pt-20"><ProblemsSection /></div>
      <div ref={faqRef} className="min-h-screen pt-20"><FAQ /></div>
    </>
  )
}
